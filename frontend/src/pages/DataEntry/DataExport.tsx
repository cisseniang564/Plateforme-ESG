...
const PILLARS = [
  { id: '',              labelKey: 'dataExport.pillarAll',    icon: BarChart3,  color: 'text-gray-500' },
  { id: 'environmental', labelKey: 'dataExport.pillarEnv',    icon: Leaf,       color: 'text-emerald-600' },
  { id: 'social',        labelKey: 'dataExport.pillarSocial', icon: Users,      color: 'text-blue-600' },
  { id: 'governance',    labelKey: 'dataExport.pillarGov',    icon: Building2,  color: 'text-purple-600' },
]

const STATUSES = [
  { id: '',          labelKey: 'dataExport.statusAll' },
  { id: 'verified',  labelKey: 'dataExport.statusVerified' },
  { id: 'pending',   labelKey: 'dataExport.statusPending' },
  { id: 'rejected',  labelKey: 'dataExport.statusRejected' },
]

const CURRENT_YEAR = new Date().getFullYear()
const YEAR_VALUES = [0, CURRENT_YEAR, CURRENT_YEAR - 1, CURRENT_YEAR - 2, CURRENT_YEAR - 3]
...
const EXPORT_FORMATS = [
  {
    ...
    descKey: 'dataExport.formatXlsxDesc',
    color: 'border-emerald-200 bg-emerald-50 hover:bg-emerald-100',
    badgeKey: 'dataExport.badgeRecommended',
  ...
  },
  {
    ...
    label: 'CSV (.csv)',
    icon: FileText,
    descKey: 'dataExport.formatCsvDesc',
    color: 'border-blue-200 bg-blue-50 hover:bg-blue-100',
    badgeKey: 'dataExport.badgeUniversal',
...
export default function DataExport() {
  ...
  const YEARS = YEAR_VALUES.map(y => ({ id: y === 0 ? '' : String(y), label: y === 0 ? t('dataExport.yearAll') : String(y) }))
  ...
  const handleExport = async (fmt: 'xlsx' | 'csv') => {
    ...
    try {
      ...
      toast.success(t('dataExport.exportSuccess', { format: fmt.toUpperCase() }))
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <BackButton to="/app/data" label={t('dataExport.backLabel')} />

      {/* ── Header ── */}
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('dataExport.title')}</h1>
      ...
      {loadingStats ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-gray-200 p-4 text-center shadow-sm">
            ...
            <div className="text-xs text-gray-500 mt-1">{t('dataExport.totalEntries')}</div>
          </div>
          <div className="bg-emerald-50 rounded-xl border border-emerald-200 p-4 text-center">
            <div className="text-3xl font-black text-emerald-600">{stats.by_pillar?.environmental ?? 0}</div>
            <div className="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><Leaf size={10} />{t('dataExport.pillarEnv')}</div>
          </div>
          <div className="bg-blue-50 rounded-xl border border-blue-200 p-4 text-center">
            <div className="text-3xl font-black text-blue-600">{stats.by_pillar?.social ?? 0}</div>
            <div className="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><Users size={10} />{t('dataExport.pillarSocial')}</div>
          </div>
          <div className="bg-purple-50 rounded-xl border border-purple-200 p-4 text-center">
            ...
            <div className="text-xs text-gray-500 mt-1 flex items-center justify-center gap-1"><Building2 size={10} />{t('dataExport.pillarGov')}</div>
          </div>
        </div>
      ) : null}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
          ...
          {t('dataExport.exportFilters')}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {/* Pillar */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('dataExport.pillar')}</label>
            <select
              value={pillar}
          ...
          {/* Year */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('dataExport.year')}</label>
            <select
              value={year}
          ...
          {/* Status */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">{t('dataExport.verificationStatus')}</label>
            <select
              value={status}
        ...
        {(pillar || year || status) && (
          <div className="mt-3 flex flex-wrap gap-2 items-center">
            <span className="text-xs text-gray-400">{t('dataExport.activeFilters')}</span>
            {pillar && <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">{t(PILLARS.find(p => p.id === pillar)?.labelKey || '')}</span>}
            {year   && <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">{year}</span>}
            {status && <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">{t(STATUSES.find(s => s.id === status)?.labelKey || '')}</span>}
            <button
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${fmt.badgeColor}`}>{t(fmt.badgeKey)}</span>
                  </div>
                  <p className="text-xs text-gray-500 leading-relaxed">{t(fmt.descKey)}</p>
                </div>
              ...
              >
                {isExporting ? (
                  ...
                  <>
                    <FileDown size={14} />
                    {t('dataExport.download')} {fmt.label}
                    {estimatedCount !== null && (
                      <span className="bg-white/20 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                        {t('dataExport.linesCount', { count: estimatedCount, formattedCount: estimatedCount.toLocaleString(numLocale) })}
                      </span>
                    )}
                  </>
                )}
      ...
      {stats?.total === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
          ...
          <h3 className="text-sm font-semibold text-amber-800">{t('dataExport.noDataTitle')}</h3>
          <p className="text-xs text-amber-600 mt-1">
            {t('dataExport.noDataDesc')}
          </p>
          <button
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        <h3 className="text-xs font-semibold text-gray-600 mb-2">{t('dataExport.exportContent')}</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs text-gray-500">
          {(t('dataExport.fields', { returnObjects: true }) as string[]).map(field => (
