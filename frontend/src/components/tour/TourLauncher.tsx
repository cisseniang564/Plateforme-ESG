  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="hidden sm:inline">{t('tour.ui.tourLabel')}</span>
        {isCompleted ? (
          ...
          <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 px-5 py-5 text-white overflow-hidden">
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full px-3 py-1">
                    ...
                    {t('tour.ui.interactiveTour')}
                  </span>
                </div>
                <h3 className="text-xl font-bold tracking-tight">{t('tour.ui.discoverTitle')}</h3>
                <p className="text-sm text-slate-300/80 mt-1 max-w-xs">
                  {t('tour.ui.discoverDesc', { minutes: totalMinutes })}
                </p>
              </div>
              <button
                onClick={() => setOpen(false)}
                aria-label={t('tour.ui.close')}
            ...
            <div className="relative mt-4 flex items-center gap-4 text-xs text-white/60">
              <span className="flex items-center gap-1.5">
                ...
                <strong className="text-white">{totalSteps}</strong> {t('tour.ui.stepsLabel')}
              </span>
              <span className="flex items-center gap-1.5">
              ...
              <span className="flex items-center gap-1.5">
                ...
                <strong className="text-white">{TOUR_CHAPTERS.length}</strong> {t('tour.ui.chaptersLabel')}
              </span>
            </div>
            ...
            <div className="relative mt-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] text-white/40 uppercase tracking-wider">{t('tour.ui.progress')}</span>
                  ...
                  <div key={chapter.id}>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-gray-900 truncate">{t(`tour.chapters.${chapter.id}.title`, chapter.title)}</p>
                          {status === 'done' && (
                            <span className="text-[10px] font-bold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full">{t('tour.ui.completed')}</span>
                          )}
                          {status === 'active' && (
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${chapter.bgColor} ${chapter.color} border ${chapter.borderColor}`}>{t('tour.ui.inProgress')}</span>
                          )}
                        </div>
                        <p className="text-xs text-gray-500 truncate">{t(`tour.chapters.${chapter.id}.description`, chapter.description)}</p>
                      </div>

                      {/* Step badge + chevron */}
                      <div className="flex-shrink-0 flex items-center gap-2">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${chapter.bgColor} ${chapter.color}`}>
                          {t('tour.ui.stepCount', { count: chapter.stepCount })}
                        </span>
                    ...
                    {isOpen && (
                      <div className="mx-2 mb-1 px-3 py-2.5 bg-white border border-gray-100 rounded-xl shadow-sm">
                        <p className="text-xs text-gray-600 mb-2">{t(`tour.chapters.${chapter.id}.description`, chapter.description)}</p>
                        <button
                          onClick={() => {
                            setOpen(false);
          ...
          <div className="border-t border-gray-100 bg-gray-50/60 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {isCompleted || isDismissed ? t('tour.ui.relaunch') : t('tour.ui.start')}
              </button>
              {(isCompleted || isDismissed) && (
                <button
                  onClick={() => { setOpen(false); startTour(); }}
                  title={t('tour.ui.restart')}
            ...
            <p className="mt-2 flex items-center gap-1.5 text-[10px] text-gray-400 justify-center">
              ...
              {t('tour.ui.escHintPre')} <kbd className="px-1 py-0.5 bg-gray-200 rounded text-[10px] font-mono">ESC</kbd> {t('tour.ui.escHintPost')}
            </p>
          </div>
        </div>
      )}
